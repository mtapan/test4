...
public class ShopDetailsControllerTest {
	...
	@Test
	public void testSaveShopInternalServer() throws Exception {
		when(shopService.saveShop((Shop)anyObject())).thenThrow(new RuntimeException());
		mockMvc.perform(post("/shopapi").contentType(MediaType.APPLICATION_JSON_VALUE).content("{\"shopName\" : \"Parijat Mithai\",\"shopAddress\" : {\"address\" : \"Sai Apex\",\"postCode\" : 411014}}")).andExpect(status().isInternalServerError());
	}
